...
This repository contains two JavaScript coding challenges:

## 🔢 Test 1: Finding Pairs with Target Sum

### 📌 Problem Statement
You are given two rotated sorted arrays, `nums1` and `nums2`, and a target integer `k`. The goal is to find all pairs of numbers (one from each array) whose sum equals the target.

### 🚀 Features
- Accepts two arrays of unique integers.
- Finds and returns all valid number pairs that sum to `k`.
- Uses a `Set` for optimized lookup.
...
### 📌 Problem Statement
You are given an encrypted string that has been encoded using a **Caesar cipher** with an unknown shift. Additionally, you have a word that you know appears in the original plaintext. The task is to find the correct shift and decrypt the message.

### 🚀 Features
- Supports both uppercase and lowercase letters.
- Maintains spaces and punctuation.
- Uses brute force to find the correct shift by checking for the known word.
